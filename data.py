...
# roidbs 是目标检测faster-cnn的一个数据结构
def print_class_histogram(roidbs):
    """
    Args:
        roidbs (list[dict]): the same format as the output of `load_training_roidbs`.
    """
    dataset = DetectionDataset() # 加载数据集
    hist_bins = np.arange(dataset.num_classes + 1) # 创建数组

    # Histogram of ground-truth objects
    # ground-truth 对象的直方图
    gt_hist = np.zeros((dataset.num_classes,), dtype=np.int) # 返回给定形状和类型的新数组，用0填充。
    for entry in roidbs:
        # filter crowd?
        gt_inds = np.where(
...
def get_all_anchors(stride=None, sizes=None):
    """
    #   在最大可能的image，偏移，悬浮框 获得所有的anchor
    ...
    Returns:
        anchors: S x S x NUM_ANCHOR x 4, where S == ceil(MAX_SIZE/STRIDE), floatbox
    ...
    # 生成(x1, y1, x2, y2)格式的锚盒的NAx4矩阵。锚
    # 以stride / 2为中心，有指定的(近似)sqrt区域
    # 尺寸，以及给定的长宽比。
...
def get_anchor_labels(anchors, gt_boxes, crowd_boxes):
    ...
    def filter_box_label(labels, value, max_num):
        ...
        if len(curr_inds) > max_num:
            ...
            # size: 决定了输出的shape. 如果给定的是, (m, n, k), 那么 m * n * k 个采样点将会被采样. 默认为零，也就是只有一个采样点会被采样回来
            # replace: 决定采样中是否有重复值
            # p: 对应着a中每个采样点的概率分布，如果没有标出，则使用标准分布。
    ...
    # 将所有重叠crowd的非忽略候选框标记为忽略
    if crowd_boxes.size > 0:

        cand_inds = np.where(anchor_labels >= 0)[0]
        cand_anchors = anchors[cand_inds]
        # 计算box集合的交叉面积,
        # 返回: 形状为[N, M]的numpy数组，表示成对ioa分数
...
def get_rpn_anchor_input(im, boxes, is_crowd):
    ...
    Returns:
        ...
        在 feature map 中，锚标签和每个像素的目标框。
        fm_labels: fHxfWxNA
        fm_boxes: fHxfWxNAx4
        NA will be NUM_ANCHOR_SIZES x NUM_ANCHOR_RATIOS
        NA = num_anchor_sizes * num_anchor_rations
    """
    boxes = boxes.copy()
...
def get_multilevel_rpn_anchor_input(im, boxes, is_crowd):
    ...
    Returns:
        ...
        每个元组包含 feature-map 中每个像素的锚标签和目标框
